...
async function translate(text, sourceLang, targetLang, dlSession, tagHandling, printResult) {
  try {
    ...
    if (splitResult && splitResult.code === 429) {
      return {
    ...
    if (!splitResult || !splitResult.result || !splitResult.result.texts) {
      throw new Error('Split text failed or invalid response from DeepL');
    }
    ...
    // BIZTONSÁGI ELLENŐRZÉS: Itt vizsgáljuk meg, hogy a válasz érvényes-e[cite: 2]
    if (!response || !response.result || !response.result.translations || response.result.translations.length === 0) {
      console.error("[ERROR] Invalid DeepL response structure:", JSON.stringify(response));
      throw new Error('Translation failed: Invalid response structure from DeepL');
    }

    let alternatives = [], translatedText = '';

    // 获取备选翻译 (Biztonságosan kezelve)
    if (response.result.translations[0].beams && response.result.translations[0].beams.length > 0) {
      response.result.translations[0].beams.forEach(beam => {
        if (beam.sentences && beam.sentences[0]) {
    ...
    if (!translatedText) {
      throw new Error('Translation failed: empty translated text');
    }
